...
export const gameAlarmEvent = (app: Application) => {
	...
	app.get(
		...
		(req: Request, res: Response) => {
			...
			const alarmRes: AlarmResponse = {
				res: res,
				alarmInfo: { time: Number(time) , diff: Number(diff) },
			};
...
export const gameStatusChanged = (gameId: string, data: IGame) => {
...
const liveTimeAndDiff = (liveGame: IGame): AlarmInfoDTO => {
	const liveTime =
		(liveGame.gameTime.period ?? 0) * 12 -
		gameClockToMinutes(liveGame.gameTime.gameClock ?? 'PT0M00.00S');
